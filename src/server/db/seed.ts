...
export function seedDatabase(db: Database, uploadsDir: string): void {
	...
	const initialStudentPassword = process.env.SEED_STUDENT_PASSWORD ?? 'student123';
	const initialWardenPassword = process.env.SEED_WARDEN_PASSWORD ?? 'warden123';
	const studentHash = hashPassword(initialStudentPassword);
	const wardenHash = hashPassword(initialWardenPassword);
