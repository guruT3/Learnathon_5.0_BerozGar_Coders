...
class MockGrievanceService implements GrievanceService {
	async listForStudent(studentId: string): Promise<Result<Grievance[]>> {
		const currentUser = authService.restore();
		if (!currentUser) {
			return delay({ ok: false as const, error: 'Authentication required.' });
		}
		if (currentUser.role === 'student' && currentUser.id !== studentId) {
			return delay({ ok: false as const, error: 'Unauthorized to view other students’ grievances.' });
		}
	...
	async listAll(): Promise<Result<Grievance[]>> {
		...
		if (currentUser.role !== 'warden') {
			return delay({ ok: false as const, error: 'Unauthorized. Wardens only.' });
		}
		const data = [...grievances]
			.sort((a, b) => b.createdAt.localeCompare(a.createdAt))
			.map(clone);
	...
	async getById(id: string): Promise<Result<Grievance>> {
		...
		if (currentUser.role === 'student' && g.studentId !== currentUser.id) {
			return delay({ ok: false as const, error: 'Unauthorized to view this grievance.' });
		}
	...
	async create(input: CreateGrievanceInput): Promise<Result<Grievance>> {
		...
		if (!currentUser || currentUser.role !== 'student') {
			return delay({ ok: false as const, error: 'Only authenticated students can file grievances.' });
		}
		const student = MOCK_USERS[currentUser.id];
		if (!student) {
			return delay({ ok: false as const, error: 'Student account not found.' });
		}
	...
	// [SECURITY FIX 4 - VUR5] Enforce role-based authorization in mock service
	async updateStatus(id: string, status: GrievanceStatus): Promise<Result<Grievance>> {
		...
		if (currentUser.role !== 'warden') {
			return delay({ ok: false as const, error: 'Unauthorized. Only wardens can modify grievance status.' });
		}
		const g = grievances.find((x) => x.id === id);
		if (!g) {
			return delay({ ok: false as const, error: `Grievance ${id} was not found.` });
...
class MockCommentService implements CommentService {
	...
	// [SECURITY FIX 4 - VUR5] Enforce authenticated user verification for comments in mock service
	async add(grievanceId: string, authorId: string, body: string): Promise<Result<Comment>> {
		...
		if (currentUser.role === 'student' && g.studentId !== currentUser.id) {
			return delay({ ok: false as const, error: 'Unauthorized to comment on this grievance.' });
		}
		const author = MOCK_USERS[currentUser.id];
		if (!author) {
